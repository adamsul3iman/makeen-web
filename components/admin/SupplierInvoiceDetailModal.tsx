...
import { fetchSupplierInvoiceDetail } from "@/lib/suppliersClient";
...
export default function SupplierInvoiceDetailModal({ invoiceId, onClose }: { invoiceId: string; onClose: () => void }) {
  ...
  useEffect(() => {
    let alive = true;
    fetchSupplierInvoiceDetail(invoiceId)
      .then((detail) => { if (alive) setInvoice(detail); })
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-4">
          ...
          {invoice ? (
            <div className="space-y-5">
              ...
              <section><h3 className="flex items-center gap-2 text-sm font-black text-foreground"><CreditCard className="h-4 w-4 text-green-700" /> سجل الدفعات</h3><div className="mt-3 space-y-2">{invoice.payments.map((payment) => <div key={payment.id} className="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-border px-3 py-3 text-sm"><div><p className="font-black">{PAYMENT_LABELS[payment.method]}</p><p className="mt-0.5 text-xs font-bold text-muted">{new Date(payment.paidAt).toLocaleString("ar-JO")} {payment.reference ? `• ${payment.reference}` : ""}</p></div><p className="font-black tabular-nums text-green-700">{formatMoney(payment.amount)}</p></div>)}{invoice.payments.length === 0 ? <p className="rounded-lg bg-slate-50 px-3 py-6 text-center text-sm font-bold text-muted">لم تُسجل دفعات بعد.</p> : null}</div></section>
            </div>
