import type { InvoiceCreatedPayload, BarcodeLabelPrintPayload, SupplierCreatePayload, SupplierInvoiceCreatedPayload, SyncQueueRecord } from "@/lib/idb";
...
import { runIstdCatchUp } from "@/lib/istdSync";
...
async function recordSalesInvoiceLedger(
  ...
  if (!supabase || event.action_type !== "INVOICE_CREATED") return { recorded: false };
  ...
  if (existing.data?.id) {
    const backfillError = await ensureInvoiceChildren(existing.data.id, itemRows, payments, storeId);
  ...
  if (inserted.error) {
    if (inserted.error.code === "23505") {
      ...
      const backfillError = await ensureInvoiceChildren(data.id, itemRows, payments, storeId);
      if (backfillError) return { recorded: false, error: backfillError };
      return { recorded: true };
    }
  ...
  const childrenError = await ensureInvoiceChildren(invoiceId, itemRows, payments, storeId);
 ...
 * ledger row. The invoice was already cleared by POST /api/istd/submit when
 * the device was online; this only backfills the columns the mirror insert
 * could not have known about at the time.
 */
async function stampCarriedIstd(
  event: SyncQueueRecord,
  storeId: string,
): Promise<void> {
  if (!supabase || event.action_type !== "INVOICE_CREATED") return;
  const payload = event.payload as InvoiceCreatedPayload;
...
async function ensureInvoiceChildren(
  ...
  const [itemRes, paymentRes] = await Promise.all([
    supabase.from("sales_invoice_items").select("id").eq("invoice_id", invoiceId).limit(1),
    supabase.from("sales_payments").select("id").eq("invoice_id", invoiceId).limit(1),
  ]);
...
async function recordExpenseLedger(
  ...
  if (!supabase || event.action_type !== "EXPENSE_RECORDED") return { recorded: false };
...
async function recordCashMovementLedger(
  ...
  if (!supabase || event.action_type !== "CASH_MOVEMENT") return { recorded: false };
...
async function mirrorSupplierUpsert(
  ...
  if (!supabase || event.action_type !== "SUPPLIER_CREATE") return { recorded: false };
...
/**
 * Mirror a committed goods-in draft (Smart Receiving, Phase 3) into the
 * supplier accounting + inventory ledgers. The event carries everything the
 * device captured offline; every step is idempotent so retries after a
 * failed ack can never double-post stock, balances, or drawer deductions.
 *
...
async function mirrorSupplierReceiving(
  ...
  if (!supabase || event.action_type !== "SUPPLIER_INVOICE_CREATED") return { recorded: false };
  ...
  if (newProducts.length > 0) {
    ...
    if (skus.length > 0) {
      const { data: existingVariants, error: bcError } = await supabase
        .from("product_variants")
    ...
    const resolveParent = async (
      ...
      const { error: rootError } = await client
        .from("products")
  ...
  if (!existing.data) {
    const rpcResult = await supabase.rpc("create_supplier_invoice", {
      p_store_id: storeId,
  ...
  if ((itemCheck.data?.length ?? 0) === 0) {
    const { error: itemError } = await supabase.from("supplier_invoice_items").insert(
  ...
  if (paymentsToRecord.length > 0) {
    ...
    for (const [index, payment] of paymentsToRecord.entries()) {
      ...
      if (!paymentCheck.data) {
        const paymentResult = await supabase.rpc("record_supplier_payment", {
          p_store_id: storeId,
  ...
  if (cashPaid > 0 && dd) {
    ...
    const { error: drawerError } = await supabase.from("expenses").upsert(
      {
        id: drawerExpenseId,
        store_id: storeId,
...
async function mirrorShortageFlag(
  ...
  if (!supabase || event.action_type !== "SHORTAGE_FLAGGED") return { recorded: false };
  ...
  const { error } = await supabase.from("shortage_flags").upsert(
    {
      store_id: storeId,
...
async function mirrorPrintJob(
  ...
  if (!supabase || event.action_type !== "BARCODE_LABEL_PRINT") return { recorded: false };
...
async function computeShiftLedgerSums(
  ...
  const [invoices, expenses, settlements, opened, cashMovements] = await Promise.all([
    supabase!.from("sales_invoices").select("cash_amount,visa_amount,cliq_amount,debt_amount,total,discount,is_return")
      .eq("store_id", storeId).eq("shift_id", shiftId),
    supabase!.from("expenses").select("amount")
      .eq("store_id", storeId).eq("shift_id", shiftId),
    supabase!.from("customer_transactions").select("amount")
      .eq("store_id", storeId).eq("shift_id", shiftId).eq("type", "SETTLEMENT"),
    supabase!.from("sync_events").select("payload")
      .eq("store_id", storeId).eq("action_type", "SHIFT_OPENED")
      .eq("payload->>shiftId", shiftId).maybeSingle(),
    supabase!.from("cash_movements").select("type,amount")
      .eq("store_id", storeId).eq("shift_id", shiftId),
  ]);
...
async function assessShiftLedgerCompleteness(
  ...
  const [invoices, settlements, expenses, cashMovements] = await Promise.all([
    invoiceIds.length > 0
      ? supabase!.from("sales_invoices").select("sync_id").in("sync_id", invoiceIds)
    ...
    expenseIds.length > 0
      ? supabase!.from("expenses").select("id").eq("store_id", storeId).in("id", expenseIds)
      : Promise.resolve({ data: [] as Array<{ id: string }>, error: null }),
    cashMovementIds.length > 0
      ? supabase!.from("cash_movements").select("id").eq("store_id", storeId).in("id", cashMovementIds)
      : Promise.resolve({ data: [] as Array<{ id: string }>, error: null }),
  ]);
...
async function finalizeShiftClose(
  ...
  if (!supabase || event.action_type !== "SHIFT_CLOSED") return { recorded: false };
  ...
  const completeness = await assessShiftLedgerCompleteness(storeId, shiftId, event);
  if (!completeness.complete) return { recorded: false, error: "shift_ledger_incomplete" };

  const sums = await computeShiftLedgerSums(storeId, shiftId, money(payload.startingCash));
...
async function applyInvoiceStock(
  ...
  if (!supabase || event.action_type !== "INVOICE_CREATED") return { applied: false };
  const payload = event.payload as {
  ...
  for (const [index, item] of items.entries()) {
    ...
    const movement = await supabase.rpc("record_inventory_movement", {
      ...
      // MAKEEN Phase 1: a sale must never be blocked by 0/negative stock.
      // Allow total_stock to decrement into negative integers (e.g. -5).
  ...
  const marked = await supabase.from("inventory_postings").upsert(
...
function clientCreatedAt(event: SyncQueueRecord): string | null {
  switch (event.action_type) {
    case "INVOICE_CREATED":
    case "DEBT_SETTLEMENT":
...
/**
 * Write-side of the event-sourcing engine.
 * Receives a batch of offline events, upserts them into `sync_events`
 * (idempotent on `sync_id`), and returns the accepted ids so the POS can
 * clear them from IndexedDB.
 *
 * Falls back to a simulated round-trip when Supabase is not configured so
...
export async function POST(request: Request): Promise<Response> {
  ...
  if (!Array.isArray(body)) {
    return Response.json({ success: false, error: "expected_array" }, { status: 400 });
  }

  const storeAccess = supabase ? await authorizedStoreId(request) : null;
  const storeId = typeof storeAccess === "string" ? storeAccess : null;

  const events: SyncQueueRecord[] = [];
  const rejected: Array<{ sync_id: string; reason: string }> = [];
  for (const raw of body as SyncEventShape[]) {
    ...
    if (!shapeValid) {
      ...
      rejected.push({ sync_id: id, reason: "invalid_shape" });
      continue;
    }

    const badField = payloadValidationError(event);
    if (badField) {
      rejected.push({ sync_id: event.sync_id, reason: badField });
      continue;
    }
  ...
  // Mock mode: simulate the network round trip, accept everything valid.
  // The latency is a dev knob (POS_SYNC_MOCK_LATENCY_MS, default 500ms) so
  // local development still sees a realistic drain time, while automated
  // perf runs zero it to measure the handler's own cost.
  if (!supabase) {
    const mockLatency = Number(process.env.POS_SYNC_MOCK_LATENCY_MS ?? "500");
    if (Number.isFinite(mockLatency) && mockLatency > 0) {
      await new Promise((resolve) => setTimeout(resolve, mockLatency));
    }
    return Response.json({
      success: true,
      synced_ids: events.map((e) => e.sync_id),
      rejected,
      received: events.length,
    });
  }

  // Live mode: every event must be stamped with its tenant.
  if (storeAccess instanceof Response) return storeAccess;
  if (!storeId) return Response.json({ success: false, error: "store_session_required" }, { status: 401 });

  // Live mode: batch upsert into sync_events, ignoring duplicates.
  ...
  if (events.length > 0) {
    ...
    // Mirror the ledgers and only ACK events whose mirror succeeded so the
    // POS retries failed ones (all mirrors are dedupe-safe). The accounting
    // ledger (sales_invoices) is written FIRST: the admin Sales Ledger reads
    // it, so a stock/debt/loyalty hiccup must never leave a completed
    // invoice invisible in the reports. Stock runs last and is best-effort —
    // deterministic per-line failures are skipped while the sale is kept.
    // SHIFT_CLOSED events are finalized in a second pass so their drawer
    // recomputation always sees every shift-bound ledger row above.
    //
    // Prepass: every SUPPLIER_CREATE mirrors before any other event. The
    // offline queue orders PENDING rows by sync_id (a UUID), not insertion
    // order, so a SUPPLIER_INVOICE_CREATED can legitimately drain before the
    // SUPPLIER_CREATE that created its vendor — mirroring the creates first
    // guarantees the invoice's `supplier_not_found` guard can never fire.
    for (const event of events) {
      if (event.action_type !== "SUPPLIER_CREATE") continue;
      const prepassResult = await mirrorSupplierUpsert(event, storeId);
    ...
    for (const event of events) {
      ...
      if (event.action_type === "INVOICE_CREATED") {
        try {
          await recordInvoiceRiskSignals(storeId, event.sync_id, event.payload, text(event.cashierName));
        } catch (error) {
          console.error(`Risk signal error for invoice ${event.sync_id}:`, error);
        }
      }
      syncedIds.push(event.sync_id);
    }

    // Second pass: recompute the closed drawer from the now-complete ledger.
    for (const event of events) {
      if (event.action_type !== "SHIFT_CLOSED") continue;
      const shiftCloseResult = await finalizeShiftClose(event, storeId);
  ...
  // Best-effort ISTD/JoFotara catch-up: invoices mirrored now (or by earlier
  // offline passes) that never got cleared online are submitted here. Runs
  // detached so the batch response is never delayed by JoFotara latency.
  if (events.some((event) => event.action_type === "INVOICE_CREATED")) {
    void runIstdCatchUp(storeId, 5);
  }
