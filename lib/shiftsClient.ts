...
/* ─── Shift item analytics (EndShiftModal tabs) ──────────────────────────── */
 ...
 * EndShiftModal analytics tabs (formerly GET /api/shifts/[id]/items).
 */
export async function fetchShiftItemAnalytics(
  shiftId: string,
  tab: "products" | "taxes",
): Promise<{ items: ShiftProductSummary[] | ShiftTaxSummary[]; totalProducts?: number }> {
  const sb = getSupabaseBrowser();
  const storeId = getTenantStoreId();
  if (!sb || !storeId) return { items: [] };
  ...
  if (!invoices || invoices.length === 0) return { items: [] };

  const invoiceIds = invoices.map((inv) => inv.id);
  const returnInvoiceIds = new Set(
    invoices.filter((inv) => inv.is_return).map((inv) => inv.id),
  );

  const { data: items, error: itemsError } = await sb
    .from("sales_invoice_items")
    .select(
      "product_id,product_name,barcode,unit_name,qty,line_subtotal,line_discount,line_total,cost_price,cost_total,gross_profit,invoice_id",
    )
    .eq("store_id", storeId)
  ...
  if (tab === "products") {
    const byProduct = new Map<string, ShiftProductSummary>();
    for (const row of rows) {
    ...
    for (const row of rows) {
      if (returnInvoiceIds.has(row.invoice_id)) continue;
      const key = row.product_id ?? `__none:${row.product_name}`;
      const seen = invoiceSets.get(key) ?? new Set<string>();
      seen.add(row.invoice_id);
      invoiceSets.set(key, seen);
    }
    for (const [key, seen] of invoiceSets) {
      const entry = byProduct.get(key);
      if (entry) entry.invoiceCount = seen.size;
    }
    const productSummary = Array.from(byProduct.values()).sort(
      (a, b) => b.totalRevenue - a.totalRevenue,
    );
    return { items: productSummary.slice(0, 20), totalProducts: productSummary.length };
  }

  const byTax = new Map<number, ShiftTaxSummary>();
  for (const row of rows) {
    const lineSubtotal = Number(row.line_subtotal) || 0;
    const lineTotal = Number(row.line_total) || 0;
    const taxAmount = Math.max(0, lineTotal - lineSubtotal);
    const taxPercent =
      lineSubtotal > 0 ? Math.round((taxAmount / lineSubtotal) * 100 * 100) / 100 : 0;
