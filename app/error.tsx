...
export default function GlobalError({
  ...
  useEffect(() => {
    console.error("[MAKEEN] Uncaught error:", error);
  }, [error]);
  ...
  return (
    <div dir="rtl" className="flex min-h-screen w-screen flex-col items-center justify-center bg-background p-6 text-center">
      ...
      <h2 className="text-xl font-black text-foreground">حدث خطأ غير متوقع</h2>
      <p className="mt-2 max-w-md text-sm font-semibold text-muted">
        {error.message || "فشل تحميل الصفحة. أعد المحاولة أو أعد تحميل التطبيق."}
      </p>
      {error.digest && (
