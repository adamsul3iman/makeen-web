-- 037_cliq_payment.sql
-- CliQ (كليك) instant-transfer payment support.
--
-- CliQ is an electronic (non-cash) payment: it is reported separately from
-- card (VISA) in Z-reports and sales reports, never enters the cash drawer,
-- and is stored on its own column so accounting can distinguish it later.

ALTER TABLE sales_invoices
  ADD COLUMN IF NOT EXISTS cliq_amount DECIMAL(12,2) NOT NULL DEFAULT 0;

ALTER TABLE sales_invoices DROP CONSTRAINT IF EXISTS sales_invoices_payment_method_check;
ALTER TABLE sales_invoices ADD CONSTRAINT sales_invoices_payment_method_check
  CHECK (payment_method IN ('CASH', 'VISA', 'SPLIT', 'DEBT', 'CLIQ', 'UNKNOWN'));
...
ALTER TABLE sales_payments ADD CONSTRAINT sales_payments_method_check
  CHECK (method IN ('CASH', 'VISA', 'DEBT', 'CLIQ', 'UNKNOWN'));

-- Surface cliq_amount in the ledger listing and summary.
