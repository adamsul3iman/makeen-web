-- 025_sales_ledger_reporting.sql
-- Server-side sales ledger reporting for accountants. The functions keep
-- filtering, pagination, totals, and tax grouping inside PostgreSQL so a
-- long reporting period does not require loading every invoice in the app.

CREATE INDEX IF NOT EXISTS idx_sales_invoices_store_payment_completed
  ON sales_invoices (store_id, payment_method, completed_at DESC);
